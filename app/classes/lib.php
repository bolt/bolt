...
function trimText($str, $length, $nbsp = false, $hellip = true, $striptags = true){
    ...
    if (!empty($matches[0])){
        // matches contains 5 indexes
        // $matches[0] contains arrays with the full contents
        $remainder = $str;
        $trimLength = 0;
        $result = "";
        while(strlen($remainder) > 0 && $trimLength < $length){
            $matchedTags = $matches[0];
            foreach($matchedTags as $matchedTag){
                list($clean, $rest) = explode($matchedTag[0], $remainder);
                $trimLength += strlen($clean);
                $trimLength += $matches[3][0][0];
                $remainder = $rest;
                //$result .= $parts[0];
                $result .= $clean . $matchedTag[0];

            }
        }
    }
    else {
        $trimLength = strlen($str);
    }
