    ...
    public static function catchFatalErrors()
    {
        ...
        if (($error['type'] == E_ERROR || $error['type'] == E_PARSE)) {
            ...
            if ($isBoltCoreError === 0) {
                $html = str_replace('%error_title%', 'Fatal Error: Bolt Core', $html);
                $html = str_replace('%info%', '', $html);
                $message = $errorblock;
            } elseif ($isVendorError === 0) {
                $html = str_replace('%error_title%', 'Fatal Error: Vendor Library', $html);
                $html = str_replace('%info%', '', $html);
                $message = $errorblock;
                ...
                $html = str_replace('%error_title%', 'Fatal Error: Bolt Extensions', $html);
                ...
                // Unknown
                $html = str_replace('%error_title%', 'Fatal Error: Bolt Generic', $html);
                $html = str_replace('%info%', '', $html);
                $message = $errorblock;
            }
