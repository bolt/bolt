...
class Permissions
{
     ...
     */
    public function checkPermission($user, $content, $roleNames, $permissionName, $contenttype = null, $contentid = null)
    {
        ...
        if(isset($contenttype)) {
            /*
                ...
                Then if $user is a valid connected user
                  ...
                  return true for granted or false for refused
            */
            $contentRoles = array_unique($this->getRolesByContentTypePermission($permissionName, $content, $contenttype, $contentid));
            ...
            if(isset($user)) {
                ...
                || intval($content->user['id'])
                && intval($content->user['id']) === intval($user['id'])
                ) {
        ...
        // keeping usual check if no contenttype given
            foreach ($roleNames as $roleName) {
                if ($this->checkRolePermission($roleName, $permissionName, $content, $contenttype, $contentid)) {
                    $this->audit(
                        "Granting '$permissionName' " .
                        ($contenttype ? "for $contenttype " : "") .
     ...
     */
    private function checkRolePermission($roleName, $permissionName, $content = null, $contenttype = null, $contentid = null)
    {
        if ($contenttype === null) {
            return $this->checkRoleGlobalPermission($roleName, $permissionName);
        } else {
            return $this->checkRoleContentTypePermission($roleName, $permissionName, $content, $contenttype, $contentid);
        }
    }
    ...
    private function checkRoleContentTypePermission($roleName, $permissionName, $content, $contenttype, $contentid = null)
    {
        $roles = $this->getRolesByContentTypePermission($permissionName, $content, $contenttype, $contentid);
     ...
     * check.
     */
    public function getRolesByContentTypePermission($permissionName, $content, $contenttype, $contentid = null)
    {
        // Here's how it works:
        ...
        if(strval($permissionName) == 'frontend' && is_array($contenttypeRoles) && in_array(self::ROLE_VIEWERS, $contenttypeRoles)) {
            if(isset($contentid) || $content instanceof \Bolt\Content) {
                ...
                $contenttypeRoles = $this->getRolesByContentPermission($permissionName, $content, $contenttype, $contentid, $contenttypeRoles);
            } else {
     ...
     */
    public function isAllowed($what, $user, $content = null, $contenttype = null, $contentid = null)
    {
        ...
        $isAllowed = $this->isAllowedRule($rule, $user, $content, $userRoles, $contenttype, $contentid);
    ...
    private function isAllowedRule($rule, $user, $content, $userRoles, $contenttype, $contentid)
    {
        switch ($rule['type']) {
            ...
            case PermissionParser::P_SIMPLE:
                return $this->isAllowedSingle($rule['value'], $user, $content, $userRoles, $contenttype, $contentid);
            case PermissionParser::P_OR:
                foreach ($rule['value'] as $subrule) {
                    if ($this->isAllowedRule($subrule, $user, $content, $userRoles, $contenttype, $contentid)) {
                        return true;
                    }
                }
    ...
    private function isAllowedSingle($what, $user, $content, $userRoles, $contenttype = null, $contentid = null)
    {
        if ($contenttype) {
            $parts = array(
                        'contenttype',
        ...
        return $this->checkPermission($user, $content, $userRoles, $permission, $contenttype, $contentid);
    }
    ...
    public function isContentStatusTransitionAllowed($fromStatus, $toStatus, $user, $contenttype, $contentid = null)
    {
        ...
        return $this->isAllowed($perm, $user, null, $contenttype, $contentid);
    }
    ...
    public function isProtected($permissionName, $contenttype, $content = null)
    {
        ...
        $effectiveRoles = $this->getRolesByContentTypePermission($permissionName, $content, $contenttype);
